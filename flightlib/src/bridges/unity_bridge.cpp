...
bool UnityBridge::getRender(const FrameID frame_id) {
  ...
  //logger_.info("Unity Bridge has %d vehicles.", pub_msg_.vehicles.size());
  for (size_t idx = 0; idx < pub_msg_.vehicles.size(); idx++) {
    ...
    //logger_.info("Unity Bridge vehicle %d has position y %f.", idx, quad_state.x[QS::POSY]);
  }
...
bool UnityBridge::addQuadrotor(std::shared_ptr<Quadrotor> quad) {
  ...
  for (size_t cam_idx = 0; cam_idx < rgb_cameras.size(); cam_idx++) {
    logger_.info("Adding camera %d to quadrotor.", cam_idx);
...
bool UnityBridge::handleOutput() {
  ...
  //logger_.info("Unity Bridge getting camera feeds for total of vehicles %d with %d cameras.", settings_.vehicles.size(), settings_.vehicles[0].cameras.size());
  ...
  for (size_t idx = 0; idx < settings_.vehicles.size(); idx++) {
    ...
    for (const auto& cam : settings_.vehicles[idx].cameras) {
      for (size_t layer_idx = 0; layer_idx <= cam.enabled_layers.size();
           layer_idx++) {
        //logger_.info("Unity Bridge has camera %d layers for vehicle %d .", layer_idx, idx);
        ...
        logger_.info("Unity Bridge getting camera feeds for vehicle %d .", idx);
          ...
          if (cam.channels == 3) {
            cv::cvtColor(new_image, new_image, cv::COLOR_RGB2BGR); //CV_RGB2BGR
          }
          unity_quadrotors_[idx]
            ->getCameras()[cam.output_index]
            ->feedImageQueue(layer_idx, new_image);
